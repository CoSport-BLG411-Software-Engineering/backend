...
def userSignup():
    if request.method == 'POST' and 'userName' in request.form and 'userPassword' in request.form:
        ...
        conn = dbHandler.get_db_connection()
        account = conn.execute('SELECT * FROM userTable WHERE userName = "' + userName + '"' ).fetchone()
        conn.commit()
        conn.close()
...
def managerSignup():
    if request.method == 'POST' and 'managerUsername' in request.form and 'managerPassword' in request.form:
        ...
        conn = dbHandler.get_db_connection()
        account = conn.execute('SELECT * FROM managerTable WHERE managerUsername = "' + managerUsername + '"' ).fetchone()
        conn.commit()
        conn.close()
